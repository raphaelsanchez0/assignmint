...
test('User can log in and navigate to settings before logging out', async ({ page }) => {
  ...
  await page.getByPlaceholder('johndoe@gmail.com').fill(process.env.NEXT_PUBLIC_TEST_USERNAME_01!);
  await page.getByPlaceholder('Password').click();
  await page.getByPlaceholder('Password').fill(process.env.NEXT_PUBLIC_TEST_PASSWORD_01!);
